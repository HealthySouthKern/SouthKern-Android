...
public class SubmissionManager extends AppCompatActivity {
    ...
    public Event getEventInfoFromDialogue() {
        ...
        if (startHour != 0) {
            try {
                ...
                LogUtility.i(TAG, "getEventInfoFromDialogue: getStart: datetime: " + date);
                year = Integer.parseInt(date.substring(0, 4));
                month = Integer.parseInt(date.substring(5, 7));
                day = Integer.parseInt(date.substring(7, 9));
                startCalendar.set(year, month, day , startHour, startMinute);
            } catch(Exception e) {
                LogUtility.e(TAG, "getEventInfoFromDialogue: edit sub parse time err: " + e);
                e.printStackTrace();
            }
        }
        ...
        if (endHour != 0) {
            try {
                ...
                LogUtility.i(TAG, "getEventInfoFromDialogue: getEnd: datetime: " + date);
                year = Integer.parseInt(date.substring(0, 4));
                month = Integer.parseInt(date.substring(5, 7));
                day = Integer.parseInt(date.substring(7, 9));
                endCalendar.set(year, month, day , endHour, endMinute);
            } catch(Exception e) {
                LogUtility.e("getEventInfoFromDialogue: edit sub parse time err", "" + e);
                e.printStackTrace();
            }
        }
        ...
        LogUtility.i(TAG, "getEventInfoFromDialogue: eventendhour: " + endHour + "");
        if (endHour != 0) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            LogUtility.e(TAG, "onCreate: get credential: Failed to create credential: " + e);
            e.printStackTrace();
        }
        ...
        mDatabase.child("submissions").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot mSnapshot : dataSnapshot.getChildren()) {
                    LogUtility.i(TAG, "onCreate: getChildren: databaseval2: " + mSnapshot.getValue());
                    Event event = EventParser.parseSingleEvent(mSnapshot.getValue());
                    eventList.add(event);
                }
                if (!eventList.isEmpty()) {
                    LogUtility.i(TAG, "onCreate: isEmpty (210): entered if not empty: 123");
        ...
        if (eventList.isEmpty()) {
            LogUtility.i(TAG, "onCreate: isEmpty (224): entered if sub: 123");
        ...
        mAdapter.setOnItemClickListener(new EventAdapter.ClickListener() {
            ...
            public void onItemClick(int position, View v) {
                LogUtility.d(TAG, "onCreate: onItemClick: onItemClick position: " + position);
                Event event = eventList.get(position);
                LogUtility.i(TAG, "onCreate: get event: eventclick: " + event);
                eventToView = event;
        ...
        approveText.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                try {
                    ...
                    LogUtility.e(TAG, "onCreate: insertEvent: error inserting event: " + e);
                    e.printStackTrace();
                }
                mDatabase.child("submissions").child(eventToView.getSummary()).removeValue();
                editEventDialogue.dismiss();
